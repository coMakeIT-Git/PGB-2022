...
class iLearnManager: ObservableObject {
    // Variables to set ilearn and length of ilearn
    private(set) var ilearn: [iLearn.Result] = []
    ...
    func fetchiLearn() async {
        ...
        do {
            ...
            let decodedData = try decoder.decode(iLearn.self, from: data)
            ...
            DispatchQueue.main.async {
                ...
                self.ilearn = decodedData.results
                self.length = self.ilearn.count
                self.setQuestion()
            }
        } catch {
    ...
    func setQuestion() {
        ...
        if index < length {
            let currentiLearnQuestion = ilearn[index]
