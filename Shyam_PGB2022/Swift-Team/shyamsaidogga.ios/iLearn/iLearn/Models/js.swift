...
struct js: Decodable {
    ...
    struct Result: Decodable, Identifiable {
        ...
        var answers: [jsAnswer] {
            do {
                // Formatting all answer strings into AttributedStrings and creating an instance of Answer for each
                let correct = [jsAnswer(text: try AttributedString(markdown: correctAnswer), isCorrect: true)]
                let incorrects = try incorrectAnswers.map { answer in
                    jsAnswer(text: try AttributedString(markdown: answer), isCorrect: false)
                }
